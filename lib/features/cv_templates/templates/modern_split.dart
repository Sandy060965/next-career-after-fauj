...
pw.Document buildModernSplit(CvTemplateData data, CvPdfFonts fonts, CvTemplateTheme theme) {
  final styles = CvTextStyles(fonts, theme);
  final onSidebar = pw.TextStyle(font: fonts.interSemiBold, fontSize: 14, color: PdfColors.white);
  final onSidebarMuted = pw.TextStyle(font: fonts.interRegular, fontSize: 8.5, color: PdfColor.fromHex('#FFFFFFB3'));
      ...
      build: (context) => [
        pw.Row(
          ...
          children: [
            pw.Container(
              ...
              child: pw.Column(
                ...
                children: [
                  ...
                  pw.Text(
                    data.fullName.trim().isEmpty ? '' : '${data.fullName}, Veteran',
                    style: onSidebar,
                  ),
                  pw.SizedBox(height: 14),
                  pw.Container(height: 1, color: PdfColor.fromHex('#FFFFFF33')),
                  pw.SizedBox(height: 14),
                  if (data.mobileNumber.isNotEmpty) pw.Text(data.mobileNumber, style: onSidebarMuted),
                  if (data.email.isNotEmpty) ...[pw.SizedBox(height: 3), pw.Text(data.email, style: onSidebarMuted)],
                  if (data.serviceLabel.isNotEmpty || data.corpsOrArm != null) ...[
                    pw.SizedBox(height: 3),
                    pw.Text([data.serviceLabel, data.corpsOrArm].whereType<String>().join(' • '), style: onSidebarMuted),
                  ],
                ],
              ),
            ),
            pw.Expanded(
              child: pw.Padding(
                padding: const pw.EdgeInsets.fromLTRB(26, 32, 26, 32),
                child: pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
